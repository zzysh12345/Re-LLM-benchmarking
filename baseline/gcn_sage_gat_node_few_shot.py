...
for epoch in range(8000):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        if acc > best_acc + min_delta:
            ...
            counter = 0  # 重置计数器
            # 可选：保存当前最佳模型
            torch.save(model.state_dict(), f'baseline/output_model/{model_name}_{task}_{dataset_name}_few_shot_best_model.pth')
        else:
            counter += 1
...
model.load_state_dict(torch.load(f'baseline/output_model/{model_name}_{task}_{dataset_name}_few_shot_best_model.pth'))
acc = evaluate_accuracy(model, data)
