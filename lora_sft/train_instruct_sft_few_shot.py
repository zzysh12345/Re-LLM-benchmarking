...
class ModelArguments:
    ...
    few_shot_path: Optional[str] = field(default="unknown")
...
def load_jsonl_files(data_path: list[str], few_shot_path) -> Dataset:
            ...
            for root, dirs, files in os.walk(root_dir):
                for file in files:
                    if 'case' in file and 'case0' not in file and 'case1' not in file:
                        continue
                    if 'with_label' in file or 'pure_structure' in file:
                        continue
    ...
    all_file_list.append(few_shot_path)

    data = []
    for file_path in tqdm(all_file_list, desc="Loading JSONL files"):
        if 'node' in file_path or 'few_shot' in file_path:
        ...
        for i in range(repeat_time):
            with open(file_path, 'r') as f:
                for line in f:
                    if count > 3000 and 'product' in file_path and 'node_classification' in file_path:
                        break
                    if count > 1000 and 'link_prediction' in file_path and 'product' in file_path:
                        break
                    if count > 2000 and 'link_prediction' in file_path:
                        break
                    count += 1
                    try:
...
def make_train_dataset(tokenizer: transformers.PreTrainedTokenizer, data_path: list[str], data_args: DataArguments, model_args) -> Dataset:
    logging.warning("Loading data...")

    dataset = load_jsonl_files(data_path=data_path, few_shot_path=model_args.few_shot_path)
...
def train():
    ...
    with training_args.main_process_first(desc="loading and tokenization"):
        ...
        train_dataset = make_train_dataset(
            tokenizer=tokenizer, data_path=data_args.data_path, data_args=data_args, model_args=model_args)
