...
def get_pubmed_casestudy(trian_perc, val_perc, test_perc, corrected=False, SEED=0):
    ...
    data.train_id = np.sort(node_id[:int(data.num_nodes * trian_perc)])
    data.val_id = np.sort(
        node_id[int(data.num_nodes * trian_perc):int(data.num_nodes * (trian_perc + val_perc))])
    data.test_id = np.sort(node_id[int(data.num_nodes * (1-test_perc)):])
...
def get_raw_text_pubmed(trian_perc, val_perc, test_perc, use_text=False, seed=0):
    data, data_pubid = get_pubmed_casestudy(trian_perc, val_perc, test_perc, SEED=seed)
    if not use_text:
        return data, None
