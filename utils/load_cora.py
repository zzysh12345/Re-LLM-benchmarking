...
def get_cora_casestudy(trian_perc, val_perc, test_perc, SEED=0):
    ...
    data.test_id = np.sort(node_id[int(data.num_nodes * (1.0 - test_perc)):])
...
def get_raw_text_cora(trian_perc, val_perc, test_perc, use_text=False, seed=0):
    data, data_citeid = get_cora_casestudy(trian_perc, val_perc, test_perc, seed)
    if not use_text:
        return data, None
